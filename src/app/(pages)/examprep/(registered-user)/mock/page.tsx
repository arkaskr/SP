...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchmockTests = async () => {
      ...
      try {
        ...
        setmockTests(data.data as mockTest[]);
      } catch (e) {
  ...
  const uniqueSubjects = useMemo(() => {
    if (!mockTests || !Array.isArray(mockTests)) return [];
    const subjects = mockTests.flatMap((test) =>
  ...
  const availableExams = useMemo(() => {
    if (!mockTests || !Array.isArray(mockTests)) return [];
    return [...new Set(mockTests.map((test) => test.exam))];
  }, [mockTests]);
  ...
  }, [mockTests, selectedExam, selectedSubject, searchQuery]);
    ...
    return (
      <div className="container mx-auto p-4">
        <div className="bg-accent py-16 text-center mb-8 rounded-lg shadow-lg">
          ...
          <p className="text-xl">
            Loading your personalized mock experience...
          </p>
        </div>
        <Loading />
      </div>
  ...
  const renderTestRow = (test: mockTest) => {
    const totalChapters = test.subjects.flatMap((s) => s.chapters).length;
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="bg-accent py-16 text-center mb-8 rounded-lg shadow-lg">
        <h1 className="text-4xl font-bold mb-4">Mock Tests</h1>
        <p className="text-xl">Master your skills with targeted mock exams</p>
      </div>
